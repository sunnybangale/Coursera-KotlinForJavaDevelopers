...
class GameBoardImplementation<T> (width: Int) : SquareBoardImplementation(width), GameBoard <T> {
     ...
     */
    override fun filter(predicate: (T?) -> Boolean): Collection<Cell> = cellValues.filter { predicate(it.value) }.map { it.key }.toList()

    /**
     * This function finds cells based on the predicate provided by the user
     */
    override fun find(predicate: (T?) -> Boolean): Cell? = cellValues.entries.find { predicate(it.value)}?.key
