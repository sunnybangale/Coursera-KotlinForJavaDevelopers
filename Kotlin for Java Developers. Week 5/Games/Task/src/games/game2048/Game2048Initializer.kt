...
object RandomGame2048Initializer: Game2048Initializer<Int> {
    ...
    override fun nextValue(board: GameBoard<Int?>): Pair<Cell, Int>? {
        ...
        val boardValue = generateRandomStartValue()
        val emptySquares = board.getAllCells().filter { board[it] == null }.toList()
        if (emptySquares.size == 0) return null
        var cellNumber = Random.nextInt(0, emptySquares.size-1)

        return Pair(emptySquares[cellNumber], boardValue)
    }
}